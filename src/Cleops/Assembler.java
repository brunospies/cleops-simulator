...
public class Assembler{
	...
	public int[] makeInstructions(String asm, int num_lines){
		int memory[] = new int[256];
                List<Label> labels_new = new ArrayList<>();
                boolean hlt_verification = true;
		int pc=0, i, j, l, c, k, pcd=0, num_words, words_max= 80; 
		...
		for(i=0;i<num_lines;i++){
			for(j=0;j<words_max;j++){
                ...
                asm = asm.replace("\n", "\n "); //Evitar problema de linhas em branco no final do código
		String[] lines = asm.split("\n"); // Separa em linhas 
                
                String[] word;
		//Gera a matriz String de palavras
		for(i=0;i<num_lines;i++){
			word = lines[i].split(" ", words_max);
			...
			for(j=0;j<num_words;j++){
				...
				if(words[i][j].startsWith("//")){words[i][j]=""; break;}
				if(words[i][j].startsWith(".")){words[i][j]=""; break;}
				if(words[i][j].startsWith(";")){words[i][j]=""; break;}
			}
		}
			...
			for(j=0;j<words_max;j++){
				...
				if(words[i][j].compareToIgnoreCase("lda")==0){
					...
					pc++;
                                        ...
                                        j++;
                                        if(words[i][j].compareToIgnoreCase("print")==0){
                                            memory[pc] = 16;
                                            pc++;
                                        }else if(words[i][j].compareToIgnoreCase("scanf")==0){
                                            memory[pc] = 20;
                                            pc++;
                                        }else{
                                            memory[pc] = 24;
